...
for (const route of urls) {
  ...
  if (!existsSync(file)) errors.push(`Sitemap route has no file: ${route}`);
    ...
    if (/nyayanest/i.test(html)) errors.push(`Old brand string found in route: ${route}`);
  }
}
...
console.log(`Verified ${urls.length} sitemap routes, image assets, and LAWPRIME branding.`);
